...
### Project Notes — `.mynotes/` локального проекта

При работе над проектом создаётся локальная директория `.mynotes/` — полный аналог глобальных `.notes/` с той же структурой и функционалом.
...
**Правила:**
- Та же INBOX-логика: файлы из `.mynotes/INBOX/` читаются → разносятся на тематические `.md` → оригиналы удаляются
- Если использовал контекст/заметку из глобальных `.notes/` при работе над проектом — копируй соответствующий `.md` файл в `.mynotes/` проекта для памяти между сессиями
- Заметки в `.mynotes/` сохраняются в репозитории проекта — контекст не теряется при переключении между проектами
...
- **Notes system** — заметки в `.notes/` (глобальные) и `.mynotes/` (локальные проекта)
...
## Автоматизация знаний: CLI / MCP / API

### Правило «Думай как умно сделать»

Перед каждым действием ИИ обязан задать себе вопросы:
- «Как умно сделать? Можно ли автоматизировать?»
- «Есть ли лучший способ? MCP вместо CLI? Скилл вместо ручного?»
- «Как сделать чтобы следующий раз было быстрее? Создать скилл/заметку?»
- «Можно ли переиспользовать уже готовое?»

### Алгоритм при работе с CLI/MCP/API

1. **Поиск до выполнения** — проверить `.myskills/skills/`, `.agents/skills/`, `.notes/`, `mcp.so`, `skills.sh`, документацию
2. **Применение найденного** — следовать правилам скилла, использовать MCP, применить заметку
3. **Создание после успеха** — если решение может повториться: создать скилл (`.myskills/skills/`) или заметку (`.notes/`)
...
- **[VoltAgent/awesome-agent-skills](https://github.com/VoltAgent/awesome-agent-skills)** — 1060+ скиллов
- **GitHub** — поиск по `agent-skills`, `claude-skills`, `codex-skills`

### MCP-серверы
- **[mcp.so](https://mcp.so/)** — самый большой каталог
- **[Smithery](https://smithery.ai/)** — установка одной командой
- **[ToolHive](https://github.com/StacklokIO/toolhive)** — продакшен: изоляция, секреты, политики
- **[Awesome-MCP-Servers](https://github.com/punkpeye/awesome-mcp-servers)** — 30+ категорий

### Дизайн и фронтенд
- **[21st.dev](https://21st.dev/home)** — React-компоненты с демо
- **[shadcn/ui](https://ui.shadcn.com/)** — доступные компоненты
- **[v0.dev](https://v0.dev/)** — AI-генерация UI
- **[ui.aceternity.com](https://ui.aceternity.com/)** — трендовые анимации
- **[motion-primitives.com](https://motion-primitives.com/)** — анимированные паттерны
- **[hyperui.dev](https://www.hyperui.dev/)** — free Tailwind
...
- Глобальные заметки сохраняются между сессиями (`.notes/`)
- Локальные заметки проекта (`.mynotes/`) — контекст не теряется при переключении проектов
- INBOX автоматически разбирает сырой контекст (и глобальный, и проектный)
...
### Безопасность (Trail of Bits + другие)
...
**Зачем:**
- **Пользователь понимает** что просить: документацию → notes, автоматизацию паттерна → skills
- **ИИ не создаёт «тупые скиллы»** — описание API методов telethon это notes, а вот «как я обычно вступаю в группы» — skill
- **Экономия токенов** — ИИ не сканирует notes автоматически, читает только когда контекст задачи релевантен
- **Библиотек много, паттернов мало** — не нужно делать скилл на каждый метод каждой библиотеки
...
- **Не сканировать vendor-директории** — `node_modules/`, `vendor/`, `dist/`, `.git/` и другие (полный список в Pre-Action Check)
...
2. Если нет → создай `AGENTS.md` с мини-шаблоном
3. Если есть другой формат правил → добавь ссылку туда же
